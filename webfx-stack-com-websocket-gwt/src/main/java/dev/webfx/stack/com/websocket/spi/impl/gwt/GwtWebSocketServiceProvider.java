...
public final class GwtWebSocketServiceProvider implements WebSocketServiceProvider {
    ...
    @Override
    public GwtWebSocket createWebSocket(String url, ReadOnlyAstObject options) {
        ...
        if (sockJS != null) { // Yes the script was included, so a sockJS has already been started
            ...
            if (options == null && Objects.equals(w.get("quickStartSockJSUrl"), url)) // Checking the parameters are the same
                return new GwtWebSocket(sockJS); // Yes! The connection is probably already established (we gained a few seconds!)
        ...
        return new GwtWebSocket(new SockJS(url, null, options));
    };
