package dev.webfx.stack.db.querypush.server.spi.impl.simple;

import dev.webfx.stack.db.querypush.server.spi.impl.ServerQueryPushServiceProviderBase;
            ...
            if (argument != null) { // argument may be null (some code can wake up pulse by calling executePulse(null))
                ...
                for (QueryInfo queryInfo : queryInfos.values()) {
                    ...
                    if (modifiedScope != null) {
                        ...
                        // If the modified scope has no intersection with the query scope, this means the modification doesn't impact the query, therefore it's not necessary to mark it as dirty
