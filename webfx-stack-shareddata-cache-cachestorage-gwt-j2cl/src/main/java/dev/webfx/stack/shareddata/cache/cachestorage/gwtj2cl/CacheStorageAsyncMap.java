...
final class CacheStorageAsyncMap implements AsyncMap<String, Object> {
    ...
    public Future<Object> get(String key) {
        return GwtJ2clUtil.jsPromiseToWebFXFuture(openCache().then(cache -> cache.match(buildUrl(key))))
            .compose(response -> {
                if (response == null) {
                    return Future.succeededFuture(null);
                }
                return GwtJ2clUtil.jsPromiseToWebFXFuture(response.text())
                    .map(text -> {
    ...
    public Future<Void> put(String key, Object value) {
        ...
        return GwtJ2clUtil.jsPromiseToWebFXFuture(openCache().then(cache -> cache.put(request, response)))
            .mapEmpty();
    }
    ...
    public Future<Void> voidRemove(String key) {
        return GwtJ2clUtil.jsPromiseToWebFXFuture(openCache().then(cache -> cache.delete(buildUrl(key))))
            .mapEmpty();
    }
    ...
    public Future<Void> clear() {
        ...
        return GwtJ2clUtil.jsPromiseToWebFXFuture(caches.delete(CACHE_NAME))
            .mapEmpty();
    }
